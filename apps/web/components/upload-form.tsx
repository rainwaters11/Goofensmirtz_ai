...
/** Sentinel UUID used only when no auth session is available (dev / demo). */
 ...
 *
 * The owner_id is sourced from the authenticated Supabase session via useUser().
 * Falls back to DEMO_OWNER_ID when running without auth (local dev / demo mode).
 *
 * @deprecated Prefer UploadCard for new usage — this form is kept for backwards
 * compatibility with any page that still imports it directly.
 */
export function UploadForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Use the authenticated user's UUID; fall back to demo sentinel in dev mode.
